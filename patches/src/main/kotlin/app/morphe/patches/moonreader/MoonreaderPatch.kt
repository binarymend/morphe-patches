...
@Suppress("unused")
val moonreaderProPatch = bytecodePatch(
    name = "Unlock Moon+ Reader Pro",
    description = "Unlocks Pro features including PDF support, ad-free experience, and more.",
    default = true
) {
    ...
    execute {
        ...
        PDF_IS_LICENSE_ACTIVATED.match(classDefBy(PDF_IS_LICENSE_ACTIVATED.definingClass!!)).method.apply {
            if (implementation == null) return@apply
            removeInstructions(0, instructions.count())
            addInstructions(0, "const/4 v0, 0x1\nreturn v0")
        }
...
@Suppress("unused")
val moonreaderUIPatch = bytecodePatch(
    name = "Make UI look like Pro",
    description = "Changes the app title to 'Moon+ Reader Pro' and removes the 'Remove Ads' button from the drawer.",
    default = true
) {
    compatibleWith(Constants.COMPATIBILITY_MOONREADER)
